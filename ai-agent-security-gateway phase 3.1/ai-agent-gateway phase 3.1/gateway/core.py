...
# pre-registered one-by-one in the permissions table. For PROCESS_CONTENT,
# the "resource" is an arbitrary block of text. Both are checked against a
# fixed placeholder resource meaning "this agent may do X at all" — the
# real resource (URL or content) is still passed through to the risk engine
# for per-request scoring.
PERMISSION_BUCKETS = {}
PERMISSION_BUCKETS.update({action: "web" for action in risk_engine.WEB_ACTIONS})
PERMISSION_BUCKETS.update({action: "content" for action in risk_engine.CONTENT_ACTIONS})

# How much of a long content/URL resource to keep in the audit log and
# API response — full text is only needed transiently, for scoring.
RESOURCE_PREVIEW_LEN = 200
...
class Gateway:
    ...
    def _has_permission(self, agent_id, action, resource):
        conn = db.get_connection()
        lookup_resource = PERMISSION_BUCKETS.get(action.upper(), resource.lower())
        row = conn.execute(
    ...
    @staticmethod
    def _display_resource(action, resource):
        """Truncate long content/URLs for storage & display — full text is
        only needed transiently, for the risk engine to scan."""
        if len(resource) > RESOURCE_PREVIEW_LEN:
            remaining = len(resource) - RESOURCE_PREVIEW_LEN
            return f"{resource[:RESOURCE_PREVIEW_LEN]}... [{remaining} more chars]"
        return resource
    ...
    def evaluate_request(self, agent_id, action, resource, scope="SINGLE"):
        ...
        display_resource = self._display_resource(action, resource)
        audit_id = self._log_audit(
            agent_id, action, display_resource, scope, risk_score, decision, all_reasons
        )
    ...
    def _log_audit(self, agent_id, action, resource, scope, risk_score, decision, reasons):
        ...
        cur = conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (agent_id, action.upper(), resource, scope.upper(), risk_score, decision, json.dumps(reasons)),
        )
        conn.commit()
