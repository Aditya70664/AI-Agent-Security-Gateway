# AI Agent Security Gateway (Phase 1 + Phase 2)
...
signal source* alongside the tool-action risk from Phase 1. When an agent
requests `VISIT` or `DOWNLOAD`, the resource is treated as a URL and
scored by `gateway/url_risk_engine.py` (IP-address hosts, "@" redirect
tricks, suspicious TLDs, link shorteners, phishing keywords, executable
downloads, etc.) instead of the generic named-resource lookup. Nothing in
Phase 1 had to change to add this — that was the point of separating
`risk_engine.py`, `policy_engine.py`, and `core.py`.

A dashboard UI is still meant to plug in later as a frontend on top of
this same API.
...
# 4. Or run it as a real HTTP API
...
- ⬜ Phase 5 — JIT (time-limited) permissions, anomaly detection,
  swap the rule-based URL scoring for a trained ML classifier
  (Logistic Regression / Random Forest on URL features) without
  changing `url_risk_engine.py`'s public interface
